...
export const PromptProvider = ({ children }: PromptProviderProps) => {
  ...
  const fetchPrompts = useCallback(
    async (params?: {
      ...
      try {
        ...
        console.error(error);
        setError(
          "We couldn't load your prompts. Check that the backend is running and try again.",
        );
      } finally {
        setLoading(false);
      }
  ...
  const editPrompt = useCallback(
    async (id: string, data: PromptFormData) => {
      try {
  ...
  const removePrompt = useCallback(
    async (id: string) => {
      try {
        ...
        console.error(error);
        toast.error("Couldn’t delete the prompt");
        throw error;
      }
    },
    [fetchPrompts],
  ...
  const duplicateExistingPrompt = useCallback(
    async (id: string) => {
      try {
        ...
        console.error(error);
        toast.error("Couldn’t duplicate the prompt");
        throw error;
      }
    },
    [fetchPrompts],
