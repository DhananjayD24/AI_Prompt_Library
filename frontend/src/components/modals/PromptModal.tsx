...
export function PromptModal({ prompt, onClose }: PromptModalProps) {
  ...
  const save = async (data: PromptFormData) => {
    try {
      if (prompt) await editPrompt(prompt._id, data);
      else await createNewPrompt(data);
      onClose();
    } catch {
      ...
      >
        <div className="modal-header">
          <div>
            ...
            <p>
              {prompt
                ? "Update the details of your saved prompt."
                : "Save a reusable prompt to your library."}
            </p>
          </div>
          <button
